 ...
 * @Version:1.0
 **/
public class BuildCodeDyMyBatisPlugsServiceImpl extends BuildCodeTemplateServiceImpl implements IBuildCodeTemplateService {

    @Override
    public void preProcessDate(TableClass tableClass,  EntityClass entityClass) {
        // 数据库字段都转换小写
        tableClass.setTableName(tableClass.getTableName());

        if(CollectionUtils.isNotEmpty(tableClass.getColumnList())){
            for(TableColumn tableColumn : tableClass.getColumnList()){
                tableColumn.setColumnName(tableColumn.getColumnName());
            }
        }
